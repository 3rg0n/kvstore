// goodclient is a test HTTP client for kvstore e2e testing.
// It is intended to be registered via "kvstore app register".
package main
...
// Unique identity ensures this binary has a different SHA-256 hash than badclient.
const appIdentity = "goodclient-v1"
...
func main() {
	...
	args := flag.Args()
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "Usage: goodclient [flags] <command> [args]\n")
